...
test('first Owner can bootstrap, log out and log back in', async ({ page, request }) => {
  await page.goto('/');
...
test('invalid credentials do not reveal account existence', async ({ page }) => {
  await page.goto('/');
  await page.getByLabel('E-posta').fill('unknown@example.com');
  await page.getByLabel('Parola').fill('IncorrectPassword2026');
  await page.getByRole('button', { name: 'Giriş Yap' }).click();

  await expect(page.getByRole('alert')).toHaveText('E-posta veya parola doğrulanamadı.');
});

...
import {
  ...
  getQuotationConversionDataWindow,
  type QuotationConversionFilters,
  type QuotationConversionReportResult,
  type QuotationConversionSourceRecord,
  type ReportBusinessUnitContract,
} from '@ertip/reporting';

export interface QuotationConversionReportQuery {
...
function assertScope(businessUnitId: string, allowedBusinessUnitIds: readonly string[]): void {
...
function mapBusinessUnit(row: BusinessUnitRow): ReportBusinessUnitContract {
  return {
    id: row.id,
...
function mapSourceRecord(row: SourceRow): QuotationConversionSourceRecord {
  return {
    id: row.odoo_id,
...
): Promise<readonly ReportBusinessUnitContract[]> {
...
export async function queryQuotationConversionReport(
  pool: Pool,
  input: QuotationConversionReportQuery,
): Promise<QuotationConversionReportResult> {
  assertScope(input.filters.businessUnitId, input.allowedBusinessUnitIds);
  ...
  if (!businessUnit) throw new Error('REPORT_BUSINESS_UNIT_UNAVAILABLE');
